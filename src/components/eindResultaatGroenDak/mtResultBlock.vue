<template>
  <v-container>
    <table>
      <thead>
        <tr>
          <th rowspan="2" style="width:25em" class="thTopR"></th>
          ...
          <th class="thTopTussen"></th>
          <th rowspan="2" style="width:10em">Klassieke Muur</th>
          <th v-if="calcColumns === 1" colspan="1" style="width:10em">
            Groene Gevel
          </th>
          <th v-if="calcColumns === 2" colspan="2" style="width:10em">
      ...
      <tbody v-for="(item, index) in infoChapter" v-bind:key="index">
        <mtResultDetail
          :klassiekGD="infoGD[item.klassiek ? item.klassiek : undefined]"
          :groenGD="infoGD[item.groen ? item.groen : undefined]"
          :klassiekGG="infoGG[item.klassiek ? item.klassiek : undefined]"
          :groenGG="infoGG[item.groen ? item.groen : undefined]"
          :min="min"
          :avg="avg"
          :max="max"
...
export default {
  props: ['infoGD', 'infoGG', 'target', 'min', 'avg', 'max'],
  data: function() {
    return {
      //waarde: 'Gemiddeld',
  ...
  computed: {
    infoChapter: function() {
      return this.infoGD[this.target];
    },
